...
<body onload=toggleVisibility(1)>
...
<div style="float:right;"><a href="<?php echo base_url()."papers/" ?>" title="" class="basicBtn">Go Back</a></div>
...
<div class="container-fluid" >
		...
		<br/>
		<br/>
		 <div><h1><strong>TESLA</strong></h1></div>
        <br/>
		<br/>
        </div><!-- End container -->
    </div><!-- End jumbotron-->
	...
	<div class="well">
		<div class="container text-justify">
			...
			<table border="1">
				...
				<col width="300">
				<tr>
				<td height="250" style="padding-right: 20px">
					<div class="inner_div">
					<h1 class="text-center text_underline"> Timer : <span id='timer'> Infinite Time</span> </h1>
	     			<br/>
                    <?php $c=0?>
                    <?php foreach($questions as $key=>$value) {$c++;?>
					<form name="paper" id="paper" action="<?php echo base_url(); ?>papers/submit/<?php echo $paper['pid'];?>" method="POST">
						<input type="hidden" name="pid" value="<?php echo $paper['pid']; ?>">
							
							<div id="<?php echo "q".$c; ?>" style="display: none;">
							
                            <h3><?php echo "Q".$c."."; ?>&emsp;<?php echo $value['question']; ?></h3>
							<?php if($value['image_url']) { ?><img src="<?php echo base_url();?>static/images/questions/<?php echo $value['image_url']; ?>" alt="Image Unavailable" float="left" width="50%" /></a><br/> <?php } ?>
                            <br />
							<label for="<?php echo "q".$c;?>op1"><input type="radio" id="<?php echo "q".$c;?>op1" name="<?php echo "qu".$value['qid'];?>" value="1">&emsp;&emsp;&emsp;<span style="font-size: 17px"><?php echo $value['option1'];?><?php if($value['op1_url']) { ?><img src="<?php echo base_url();?>static/images/option1/<?php echo $value['op1_url']; ?>" alt="Image Unavailable" float="left" width="50%" /></a><br/> <?php } ?></span></label>
                            <br /><br />
                            <label for="<?php echo "q".$c;?>op2"><input type="radio" id="<?php echo "q".$c;?>op2" name="<?php echo "qu".$value['qid'];?>" value="2">&emsp;&emsp;&emsp;<span style="font-size: 17px"><?php echo $value['option2'];?><?php if($value['op2_url']) { ?><img src="<?php echo base_url();?>static/images/option2/<?php echo $value['op2_url']; ?>" alt="Image Unavailable" float="left" width="50%" /></a><br/> <?php } ?></span></label>
                            <br /><br />
                            <label for="<?php echo "q".$c;?>op3"><input type="radio" id="<?php echo "q".$c;?>op3" name="<?php echo "qu".$value['qid'];?>" value="3">&emsp;&emsp;&emsp;<span style="font-size: 17px"><?php echo $value['option3'];?><?php if($value['op3_url']) { ?><img src="<?php echo base_url();?>static/images/option3/<?php echo $value['op3_url']; ?>" alt="Image Unavailable" float="left" width="50%" /></a><br/> <?php } ?></span></label>
                            <br /><br />
                            <label for="<?php echo "q".$c;?>op4"><input type="radio" id="<?php echo "q".$c;?>op4" name="<?php echo "qu".$value['qid'];?>" value="4">&emsp;&emsp;&emsp;<span style="font-size: 17px"><?php echo $value['option4'];?><?php if($value['op4_url']) { ?><img src="<?php echo base_url();?>static/images/option4/<?php echo $value['op4_url']; ?>" alt="Image Unavailable" float="left" width="50%" /></a><br/> <?php } ?></span></label>
							</div>
						<?php } ?>
					<!--<input type="submit" name="submit and exit" value="Submit">	-->
					</form>
					</div>
					
				</td>
				<td height="250" style="padding: 10px"><h3><label>&nbsp&nbsp Question Pallet :</label></h3>
					<table>
						<?php
							$n = count($questions);
							for($i=1; $i<=$n; $i++) {	
						...
						<?php if($i%4 == 1) { ?> <tr> <?php } ?>
							<td>
								<button id="x<?php echo $i; ?>" class="btn btn-danger" onclick="toggleVisibility(<?php echo $i; ?>);">Q <?php echo $i; ?></button>
							</td>
						<?php if($i%4 == 0) { ?> </tr> <?php } ?>
						<?php } ?>
	...
	<div class="well">
		...
		<form>	
			<table width="100%">
				<tr>
					<!--<td width="25%"></td>-->
					<td width="25%"><a href="#" onclick="prev()">
					<button type="button" name="prev" class="btn btn-primary">PREVIOUS</button></td></a>
                    <td width="25%"><a href="#" onclick="flag()">
                    <button  type="button" name="sub" class="btn btn-warning">FLAG</button></td></a>
					<td width="25%"><a href="#" onclick="next1()">
					<button  type="button" name="sub" class="btn btn-success">SAVE & CONTINUE</button></td></a>
					<td width="25%"><a href="#" onclick="next()">
					<button type="button" name="nxt" class="btn btn-primary">NEXT</button></td></a>
                    <!--<td width="20%"><a href="#" onclick="()">-->
...
<div align="center">
	<button type="button" name="submit" style=" font-size:36px;background-color:#000;height:100px; width:400px" class="btn btn-primary" onclick=$("#paper").submit(); >SUBMIT & EXIT</button></td>
</div>
</section>
...
var n= <?php echo count($questions);?>;
   ...
   function flag()
   {
	   var dd="x"+counter;
	    document.getElementById(dd).classList.remove("btn-danger");
		 document.getElementById(dd).classList.remove("btn-info");
		  document.getElementById(dd).classList.remove("btn-success");
		   document.getElementById(dd).classList.add("btn-warning");
   }
    function next()
    {
        var dd="x"+counter;
    	if(counter!=n)
          ...
          for(var i=1;i<=4;i++)
        {
            var pandey="q"+counter+"op"+i;
          
            if(document.getElementById(pandey).checked=="true")
                document.getElementById(pandey).checked="false";
     ...
     function next1()
    {
        ...
        if(set==0)
            {
                alert("Please select a option to save and continue!!!");
                return;
    	...
    	document.getElementById(dd).classList.remove("btn-danger");
		document.getElementById(dd).classList.remove("btn-info");
		 document.getElementById(dd).classList.remove("btn-warning");
...
<script type="text/javascript">
    ...
    {
        return 'Your Answered Questions will be submitted if pressing SUBMIT & EXIT else resetted to ZERO, Please select stay on page to continue your Quiz';
    }
